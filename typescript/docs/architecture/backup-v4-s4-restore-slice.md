...
Empty-attribute component trees and integer-scored risk rows are admitted below.
Non-empty component attributes, constraint values, task dependencies, event payloads,
and every other record kind stay refused.
...
The same unregistered adapter now admits two more current `projectRecords` kinds.
Logical record ids stay verbatim. Convex physical ids continue to use the existing
table-scoped S4 identity list. `UuidRemapper` is not used: this path does not mint
logical ids, and a parent string that happens to equal a source `_id` is an
unresolved edge rather than a physical id to translate.

Admitted:

- `component` rows whose `attributes` object is empty, whose `name` / `type` /
  `status` are canonical non-empty text, and whose `parentComponentId` is either
  `null` or another component `recordId` in the same project. Parent cycles,
  self-parents, cross-project parents, and parents that only match a risk or a
  physical id are refused. `notes` is opaque text and is not scanned for ids.
  Component `status` is a label, not a lease.
- `risk` rows whose `hazard` and `controls` are canonical text and whose
  `likelihood` and `consequence` are integers from 1 to 5. Fractional or
  out-of-range scores are refused. Restoring a risk does not execute its controls.

Still refused, including when the payload looks simple:

- any component `attributes` key, because the writer stores `v.any()` and no
  producer classification exists;
- `constraint` rows, because `value` is unclassified;
- `task` rows, including `done` tasks with an empty `dependencies` array,
  because those strings are an unresolved reference list and status is
  operational;
- `event` rows, including an empty `payload`;
- component or risk definitions inside `memoryChangeSets` (the memory producer
  only accepts fact, assumption, measurement, and decision).

Ordinary `projectRecords.listByKind` readback and the tagged digest cover these
