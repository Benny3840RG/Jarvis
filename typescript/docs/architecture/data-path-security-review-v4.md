...
| Parsing — symlinks                                 | Refused at both `lstat` and `open` (`O_NOFOLLOW`) for every source, archive and destination path.                                                                                                                                                                                         |
...
| Mutation — never touches live data                 | `assertDestinationNotLive` refuses a destination that is inside, or contains, the live data directory. A restore writes only inside the directory it reserved — asserted by a test that the destination's parent holds nothing else.                                                      |
...
| Mutation — bounded recovery                        | `--resume` computes its deletable set from the archive, not from disk; any entry the restore did not write makes it refuse and delete nothing.                                                                                                                                            |
