...
The optional `POSTHOG_HOST` defaults to `https://us.i.posthog.com`. It must be HTTPS with no embedded credentials, query string or fragment. The adapter posts to the current `/i/v0/e/` capture endpoint with the fixed development distinct ID in the required top-level field. `POSTHOG_TIMEOUT_MS` is optional and defaults to 250 ms. Ordinary request-path telemetry accepts 25–2000 ms and keeps no more than 32 events in flight; overflow is dropped and counted as failed delivery. The isolated `npm run commission:posthog` command alone accepts up to 10000 ms for slower development links and fails unless PostHog acknowledges all three governed health events. Invalid configuration fails closed to inert telemetry. The project key is used only for the PostHog capture transport and is never included in event properties.
