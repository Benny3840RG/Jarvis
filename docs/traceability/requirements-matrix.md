...
Statuses above retain the last full implementation/evidence reconciliation baseline at 924bda923f540ede6b9a80eaf70ab115cff03969. The current ΩΣ branch was cut from main at 0fcad08daa2158cab8183e0d65534501e861438c; post-baseline evidence is recorded below without inflating broad requirement ranges beyond what has actually been commissioned. Partial means the repository contains implementation, tests, runbooks, or governance evidence, but the requirement range is not fully closed. Unverified means this reconciliation found no current evidence sufficient to assess the range. A Partial status is not a commissioning or production claim.
...
### ΩΣ governed mission runtime — 2026-08-10

PR #363 adds a durable owner-scoped ΩΣ mission/control plane without replacing Jarvis's existing governed execution boundary. The implementation adds mission, evidence, validation-proof, and action-contract records; binds destructive/single-use ΩΣ contracts to existing approved ToolActions; enforces ΩΣ authorization in the same Convex transaction as `claimSingleUseExecution`; turns authoritative terminal receipts into mission evidence; and leaves indeterminate outcomes unresolved until the existing external reconciliation runtime reaches a terminal outcome. Completion reads are bounded and fail closed. R3/R4 completion forbids criterion waivers, requires independent proof tied to the criterion's evidence, preserves cumulative criterion evidence across multiple proofs, rejects missing/expired evidence, and blocks while contracts remain unresolved. Receipt reconciliation additionally verifies project, tool, operation, and the authoritative single-use execution claim before creating ΩΣ terminal evidence; identity mismatch marks the contract conflicted instead of trusting the receipt.

The exact code head e42c87c674953f73d65b3eb5c8f4f7194148efd9 passed TypeScript checks workflow 31382401252: dependency audit, TypeScript, ESLint, Prettier, OpenAPI validation, Node coverage tests, Convex tests, console build/type-check, and automation policy. Copilot Review Check 31382401438 also passed. This increases evidence for R-073–R-076, R-091–R-104, R-105–R-111, R-128–R-135, and R-141–R-143, but those broad ranges remain Partial: reusable ΩΣ contracts still need an explicit execution-generation model, and live Convex development sync plus real runtime/provider commissioning have not been proven by this PR.
