...
# Read as UTF-8 explicitly: without a UTF-8 locale set (true of some CI
# runners and this sandbox), Ruby's default external encoding falls back to
# US-ASCII, which tags file content in a way that makes String#== spuriously
# false against a UTF-8 literal even when the underlying bytes are identical.
data = YAML.safe_load(File.read(REGISTRY, encoding: "UTF-8"), permitted_classes: [], aliases: false)
errors = []
...
lines << ""
lines << "| ID | Action | Status | Domain | Capability | Overlay | Effect | Approval | External side effect | Reconciliation |"
lines << "|---|---|---|---|---|---|---|---|---|---|"
rows.each do |family|
  ...
  classification = family.fetch("classification")
  lines << "| #{family.fetch("id")} | #{family.fetch("name")} | #{family.fetch("lifecycle_status")} | #{classification.fetch("domains").join(", ")} | #{classification.fetch("capability")} | #{family.fetch("policy_overlay")} | #{effect} | #{approval} | #{side_effect} | #{reconciliation} |"
end
...
if CHECK_ONLY
  ...
  abort "#{OUTPUT} is stale; regenerate it" unless File.read(OUTPUT, encoding: "UTF-8") == rendered
  puts "Action-family registry and generated action map validated."
else
  File.write(OUTPUT, rendered, encoding: "UTF-8")
