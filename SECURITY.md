...
- Credentials settings expose short fingerprints and rotation runbooks on the authenticated JSON read model. The public loopback page also embeds SHA-256 digests of the current and previous service tokens for a local collision check; those digests are not raw tokens and are not in the JSON read model, MCP tools, or the operator widget. A full secret is shown at most once on that loopback page. End overlap returns CLI text only and does not remove a token. There is no control that enables remote exposure on its own.
