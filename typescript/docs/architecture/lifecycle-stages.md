...
| Business backup coverage (A2)   | `implemented`, partial v4      | v4 captures and verifies core, memory and JSON businessRecords including business settings; v1–v3 remain available. See [the v4 contract](backup-v4-contract.md) and [domain inventory](authoritative-domain-inventory.md). | Convex notesAndEvidence and quoteAggregate/blob groups remain absent from export-v4. Isolated adapters restore a closed notes/evidence subset (including empty-attribute components and bounded risks) and closed terminal orchestration only; neither group is sealed. Full recovery refuses partial archives. |
