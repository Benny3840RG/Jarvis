...
The maintained orchestration foundation now also has immutable graph state, deep-frozen trigger payloads, retryable budget-stop classification, and corrected runtime/traceability status. It remains partial: durable run state, restart recovery, idempotent trigger consumption, cancellable effects, and runtime composition are not yet evidenced.
