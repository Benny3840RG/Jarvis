...
export function validateWorkflowContract(workflow) {
  ...
  const requirements = [
    ...
    ["workflow must run candidate verification job", /^\s{2}verify-candidate:\s*$/m],
    [
      "candidate verification must wait on PR-scoped checks",
      /listForRef/i,
    ],
  ...
  const verifyJob = text.match(/^\s{2}verify-candidate:[\s\S]*?(?=^\s{2}[a-zA-Z_-]+:\s*$)/m)?.[0] ?? "";
  if (/actions\/checkout@/i.test(verifyJob)) {
    reasons.push("candidate verification must not check out repository content");
  }
  if (/\bnpm\s+ci\b/i.test(verifyJob) || /\bnpm\s+run\b/i.test(verifyJob)) {
    reasons.push("candidate verification must not execute npm on the untrusted tree");
  }
  if (/needs\.build\.outputs\.candidate-sha/i.test(verifyJob) && /actions\/checkout@/i.test(verifyJob)) {
    reasons.push("candidate verification must not check out the untrusted candidate SHA");
  }
