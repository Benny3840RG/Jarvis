...
The maintained orchestration foundation on PR #322 adds validated trigger shape, immutable weighted graph scheduling, and fail-closed step/duration budgets. It remains a partial implementation: durable run state, restart recovery, idempotent trigger consumption, cancellable effects, and runtime composition are not yet evidenced.
