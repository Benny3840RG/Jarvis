...
The current baseline is `f71a8f4d16072919c9730ba93989f4e4cd8e4e95`.
...
- The Priority 1 repository-cleanup recurrence is exposed separately by PR #394: `typescript/dist` was reintroduced as tracked output despite existing `.gitignore` guards. #394 removes that output and adds a `git ls-files` hygiene gate so tracked generated/scaffold/PR-body artefacts fail CI. It is verified on exact head `cfc184e6f0beeb4929fff2e8183a1191d4597a04` by TypeScript checks `32724822072` and Copilot Review Check `32724886933`, but is not represented here as landed until explicit per-PR landing approval and merge occur.
