...
The local JSON provider is a local fallback and recovery provider. It uses atomic replacement for complete documents and an exclusive adjacent lock file for mutations. A mutation must acquire the lock, re-read the latest document, apply one change, write atomically, and release the lock. Healthy reads do not take the lock because atomic replacement guarantees a complete old or new document. A read that detects corruption must acquire the writer lock and re-read before quarantining: another writer may already have replaced the bad file. This rule applies to core state and all business and memory JSON stores. A held writer lock can therefore cause corruption recovery to time out; the reader leaves the file in place. Already-locked mutations and snapshots perform recovery under their existing lock.
