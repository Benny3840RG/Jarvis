...
| R-091–R-098C | `safetyCategoryMatrix.test.ts`, `convex/orchestrationState.test.ts` | `src/safety/safetyBinder.ts`, `convex/orchestrationState.ts`, exact PR #329 workflow #1413 (31256419743), PR #336 workflow #1440 (31257990451) | Partial |
| R-099–R-104 | `convex/orchestrationState.test.ts`, `convex/externalReconciliations.test.ts` | `convex/orchestrationState.ts`, `convex/externalReconciliations.ts`; provider-authenticated offline recovery is now bound, while live restart drills remain open | Partial |
...
The tests prove the six-category in-process contract, fail-closed negative paths, lifecycle attachment, fresh Convex readback, durable runner composition, and provider-authenticated offline recovery binding. They do not prove authenticated worker identity, real ingress, deployed restart behavior, or live external commissioning. Status remains partial until those gates are separately evidenced.
