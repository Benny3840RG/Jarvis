...
**Fix:** `genReqId` now uses `configuredSecrets(config)`, rejecting request IDs
that contain any configured credential, including as a substring. Empty optional
credentials are ignored. The gateway rejection hook and the Nest filter share
`redactedRequestPath`; literal and partially or fully percent-encoded credential
characters are redacted without decoding unrelated path content.

The percent-aware matcher uses a deterministic scanner with an explicit work
budget. If long or ambiguous input exhausts that budget, the entire output
field becomes the redaction marker. It never returns the unchecked input.
Regression coverage runs the adversarial long-percent case in a subprocess
with a timeout, without a wall-clock performance assertion. Safe problem
details retain their non-secret multiline text.
...
none survives into a problem-details response, including explicit safe details
and early gateway rejections; a benign request id is still honoured.
...
| Auth failure logging                | HTTP rejections log source IP and rejection; Convex logs generic rejection only — neither logs candidate or configured token (`serviceTokenGuard.ts`, `authHelpers.ts`, both with comments stating this is deliberate).                                                                                         |
...
5. **Non-null assertion on a persisted field.** `snapshot.revision.fingerprint!`
   in `quoteFinalization.ts` is erased at runtime and does not itself throw or
   validate the field. Any additional runtime validation belongs with the
   upstream quote-artifact invariant; no runtime failure is established here.
