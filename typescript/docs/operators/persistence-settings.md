...
Provider selection stays in `.env.local` (`PERSISTENCE_PROVIDER`). The tab shows JSON or Convex with disabled radios and does not switch them. Convex failure does not offer a JSON fallback. The health glance is provider plus health chips and a refresh. Archive v4 export is refused while Convex is selected. v4 stays labeled Partial / JSON-only. Export copy states that archives must not contain service, approval, or delivery tokens. Classic restore requires the empty-target confirmation. v4 restore requires `--allow-partial` while the archive is partial. Resume is a separate action and is never the default after a failure.
