...
export class OrchestrationGraph {
  ...
  orderedNodes(): readonly OrchestrationNode[] {
    ...
    const ready = this.commandNodes.filter((node) => (node.dependsOn ?? []).length === 0).map((node) => node.id);
    const sortReady = (): void => {
      ready.sort((leftId, rightId) => {
        ...
        return weightDifference || (declarationOrder.get(leftId) ?? 0) - (declarationOrder.get(rightId) ?? 0);
      });
    };
    ...
    if (ordered.length !== this.commandNodes.length) {
      const unresolved = this.commandNodes.find((node) => !ordered.some((item) => item.id === node.id));
      throw new Error(`Orchestration graph contains a cycle involving ${unresolved?.id ?? "unknown"}.`);
    }
