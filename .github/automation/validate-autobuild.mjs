...
export function validateWorkflowContract(workflow) {
  ...
  const requirements = [
    ...
    [
      ...
      /actions\/permissions\/workflow[\s\S]*workflowPermissions\?\.(?:can_create_pull_request)/i,
    ],
    [
      "workflow must preflight Actions workflow-approval permissions",
      /actions\/permissions\/workflow[\s\S]*workflowPermissions\?\.(?:can_approve_pull_request_reviews)/i,
    ],
    [
