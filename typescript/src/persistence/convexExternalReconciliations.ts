...
function reconciliationFromConvex(row: ReconciliationRow): ExternalReconciliationRecord {
  return {
    ...
    ...(row.terminalStatus === undefined ? {} : { terminalStatus: row.terminalStatus }),
    ...(row.resolutionDigest === undefined ? {} : { resolutionDigest: row.resolutionDigest }),
    ...
    ...(row.escalationReason === undefined ? {} : { escalationReason: row.escalationReason }),
  ...
  implements ExternalReconciliationStore, ExternalReconciliationReadStore
{
  ...
  constructor(
    ...
    if (!serviceToken) throw new Error("External reconciliation requires JARVIS_SERVICE_TOKEN.");
    ...
    if (!convexUrl) throw new Error("External reconciliation requires CONVEX_URL.");
  ...
  async getForOperator(reconciliationId: string): Promise<ExternalReconciliationEnvelope | null> {
  ...
  async markIndeterminate(
    ...
    const row = await this.client.mutation(externalReconciliationFunctions.markIndeterminate, {
      serviceToken: this.serviceToken,
      ...scopeArgs(input),
      reconciliationId: input.reconciliationId,
      executionKey: input.executionKey,
  ...
  async releaseClaim(input: {
    ...
    const row = await this.client.mutation(externalReconciliationFunctions.releaseClaim, {
      serviceToken: this.serviceToken,
      ...input,
    });
    return reconciliationFromConvex(row as ReconciliationRow);
